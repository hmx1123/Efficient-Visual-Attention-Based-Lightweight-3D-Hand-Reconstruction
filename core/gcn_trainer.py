    ...
    if cfg.TRAIN.OPTIM == "adam":
        if dist_training:
            optimizer = ZeroRedundancyOptimizer(
                optim_params, optimizer_class=torch.optim.Adam, lr=cfg.TRAIN.LR
            )
        else:
            optimizer = torch.optim.Adam(optim_params, lr=cfg.TRAIN.LR)
    elif cfg.TRAIN.OPTIM == "adafactor":
        if dist_training:
            optimizer = ZeroRedundancyOptimizer(
                optim_params,
        ...
        if dist_training:
            optimizer = ZeroRedundancyOptimizer(
                optim_params, optimizer_class=torch.optim.RMSprop, lr=cfg.TRAIN.LR
            )
        else:
