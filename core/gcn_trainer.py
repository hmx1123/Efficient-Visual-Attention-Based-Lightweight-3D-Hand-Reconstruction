...
def train_gcn(rank=0, world_size=1, cfg=None, dist_training=False):
    ...
    trainDataset = handDataset(mano_path=mano_path,
                               ...
                               theta=[-cfg.DATA_AUGMENT.THETA,
                                      cfg.DATA_AUGMENT.THETA],
                               scale=[1 - cfg.DATA_AUGMENT.SCALE,
                                      1 + cfg.DATA_AUGMENT.SCALE],
    ...
    for hand_type in ['left', 'right']:
        ...
        J_regressor = torch.sparse_coo_tensor(
            i.t(), v, torch.Size([16, 778])).to_dense()
    ...
    for epoch in range(cfg.TRAIN.current_epoch, cfg.TRAIN.EPOCHS):
        ...
        print(f"epoch_loss:{np.mean(epoch_loss)},lr:{cfg.TRAIN.LR}")
        lr_scheduler.step()
