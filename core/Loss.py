...
MANO_PARENT = [-1, 0, 1, 2, 3, 0, 5, 6, 7, 0, 9, 10, 11, 0, 13, 14, 15, 0, 17, 18, 19]


class GraphLoss:
    def __init__(self, J_regressor, faces, level=4, device="cuda"):
        # loss function
    ...
    def norm_loss(self, verts_pred, verts_gt):
        ...
        edge_pred = torch.stack(
            [
                edge_pred[:, :, 0] - edge_pred[:, :, 1],
    ...
    def calc_mano_loss(self, v3d_pred, v2d_pred, v3d_gt, v2d_gt, img_size):
        ...
        loss_dict = {}
        loss_dict["vert2d_loss"] = self.L2Loss(
            (v2d_pred / img_size * 2 - 1), (v2d_gt / img_size * 2 - 1)
        )
    ...
    def calc_loss(
        self, converter, v3d_gt, v2d_gt, v3d_pred, v2d_pred, v3dList, v2dList, img_size
    ):
        assert self.faces.device == v3d_gt.device
        assert self.faces.device == v3d_pred.device
        mano_loss_dict = self.calc_mano_loss(
            v3d_pred, v2d_pred, v3d_gt, v2d_gt, img_size
        )
        ...
        for i in range(len(v2dList)):
            ...
            coarsen_loss_dict["v3d_loss"].append(self.L1Loss(v3dList[i], v3dList_gt[j]))
            coarsen_loss_dict["v2d_loss"].append(
                self.L2Loss(
                    (v2dList[i] / img_size * 2 - 1), (v2dList_gt[j] / img_size * 2 - 1)
                )
            )
    ...
    def range_loss(self, label, Min, Max):
        l1 = self._zero_norm_loss(torch.clamp(Min - label, min=0.0))
        l2 = self._zero_norm_loss(torch.clamp(label - Max, min=0.0))
        return l1 + l2
...
def calc_aux_loss(cfg, hand_loss, dataDict, mask, dense, hms):
    loss_dict = {}
    total_loss = 0
    if "mask" in dataDict:
    ...
    if "dense" in dataDict:
        loss_l = hand_loss.smoothL1Loss(dataDict["dense"][:, :3], dense * mask[:, :1])
        loss_r = hand_loss.smoothL1Loss(dataDict["dense"][:, 3:], dense * mask[:, 1:])
        loss_dict["dense_loss"] = (loss_l + loss_r) / 2
        total_loss = (
            total_loss + loss_dict["dense_loss"] * cfg.LOSS_WEIGHT.AUX.DENSEPOSE
        )
    if "hms" in dataDict:
